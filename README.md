...
- Catálogo estruturado: 2.561 campos tratados, com 2.509 alterações de texto. As 16 primeiras estruturas foram examinadas; cinco estruturas de identificadores ou gráficos foram preservadas integralmente. Os 12 modelos de negócio de `BMC_DATA` estão cobertos (1.826 campos), incluindo marcos, mudanças estratégicas e seletores de remoção. Também foram tratados os cinco modelos de receita, os 12 perfis de fundadores (216 campos) e suas 12 descrições de fomento. O restante do conteúdo ainda não foi traduzido por completo.
...
Os perfis de fundadores usam `node tools/catalogue-founder-profiles.cjs` e `node tools/catalogue-founder-grants.cjs`, seguidos de `npm run verify`. A validação de 20/09/2026 aprovou **91 testes**, incluindo 48 cenários de bonificação dos fundadores. A revisão independente passou de **9,2 para 9,3/10 em dois ciclos**, dentro do limite de três definido pelo usuário. Consulte [o parecer dos fundadores](docs/REVISAO_2026-09-20.md). As localidades foram tratadas na etapa seguinte, descrita abaixo.

As 36 opções de `LOCATIONS` receberam 276 entradas de catálogo (236 alterações de texto); `LOCATION_MAP` foi preservado. Execute `node tools/catalogue-locations.cjs` para regenerar o catálogo. Nomes e regiões internos permanecem iguais, pois controlam regras; os cartões, detalhes e cabeçalho apresentam exônimos e regiões em português. A validação aprovou **94 testes e 19 scripts**, incluindo 7.560 avaliações de fomento comparadas com o original. Revisão independente: **9,1 → 9,3/10 em dois ciclos**. A conferência visual das localidades ficou pendente após falhas da automação do navegador. Consulte [o parecer das localidades](docs/REVISAO_LOCALIDADES_2026-09-20.md).
