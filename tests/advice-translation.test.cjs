...
test('first four advice profiles cover exactly 90 paths and preserve weights, effects, other profiles and funding option IDs',()=>{
...
test('actual choice scoring returns the same recommendation for four initial investors across domains and choices',()=>{
 const sets=[
...
test('advice generation rejects missing translations and changed source without modifying catalogue',()=>{
 const dir=fs.mkdtempSync(path.resolve('artifacts/advice-generator-'));
 for(const folder of ['tools','source','locales'])fs.mkdirSync(path.join(dir,folder),{recursive:true});
 for(const f of ['catalogue-advice-initial.cjs','structured-translation.cjs'])fs.copyFileSync(path.join('tools',f),path.join(dir,'tools',f));
 ...
 const bad=plain(targets);bad.dragon.pop();
 fs.writeFileSync(path.join(dir,'locales/advice-initial-targets.pt-BR.json'),JSON.stringify(bad));
 const run=()=>spawnSync(process.execPath,['tools/catalogue-advice-initial.cjs'],{cwd:dir,encoding:'utf8',timeout:30000});
 ...
 fs.writeFileSync(path.join(dir,'locales/advice-initial-targets.pt-BR.json'),JSON.stringify(targets));
 fs.writeFileSync(path.join(dir,'source/index.original.html'),original.replace('Visibility is everything in the early days.','Visibility is irrelevant in the early days.'));
