...
const ids=['priyaSharma','thomasEriksson'];
...
test('two healthcare advice profiles cover exactly 40 paths and preserve weights, effects, other profiles and funding option IDs',()=>{
...
test('actual choice scoring returns the same recommendation for two healthcare investors across domains and choices',()=>{
 const sets=[
...
test('healthcare advice generation rejects missing translations and changed source without modifying catalogue',()=>{
 const dir=fs.mkdtempSync(path.resolve('artifacts/advice-healthcare-generator-'));
 for(const folder of ['tools','source','locales'])fs.mkdirSync(path.join(dir,folder),{recursive:true});
 for(const f of ['catalogue-advice-healthcare.cjs','structured-translation.cjs'])fs.copyFileSync(path.join('tools',f),path.join(dir,'tools',f));
 const original=fs.readFileSync('source/index.original.html','utf8'),catalogue=fs.readFileSync('locales/structures.pt-BR.json');
 const targets=JSON.parse(fs.readFileSync('locales/advice-healthcare-targets.pt-BR.json','utf8'));
 ...
 const bad=plain(targets);bad.priyaSharma.pop();
 fs.writeFileSync(path.join(dir,'locales/advice-healthcare-targets.pt-BR.json'),JSON.stringify(bad));
 const run=()=>spawnSync(process.execPath,['tools/catalogue-advice-healthcare.cjs'],{cwd:dir,encoding:'utf8',timeout:30000});
 ...
 fs.writeFileSync(path.join(dir,'locales/advice-healthcare-targets.pt-BR.json'),JSON.stringify(targets));
 fs.writeFileSync(path.join(dir,'source/index.original.html'),original.replace('Health economics evidence is non-negotiable.','Health economics evidence is irrelevant.'));
